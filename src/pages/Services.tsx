...
const Services = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pt-30">
      <h1 className="text-4xl font-bold mb-4 text-[#7D6855]">Our Services</h1>
      ...
      {services.map((category, idx) => (
        <div key={idx} className="mb-10">
          <h2 className="text-2xl font-semibold mb-4 text-[#7D6855]">{category.category}</h2>
          {category.items.map((service, i) => (
            ...
            >
              <div className="flex-1 text-[#7D6855]">
      ...
      {selectedService && (
        <div className="fixed inset-0 bg-white/30 backdrop-blur-sm flex justify-center items-center">
          ...
          >
            <h2 className="text-xl font-bold mb-2 text-[#7D6855]">{selectedService.service.name}</h2>
            <p className="text-sm text-gray-500 mb-2 ">
              {selectedService.service.duration} - {selectedService.service.price}
            </p>
            <p className="mb-4 text-[#7D6855]">{selectedService.service.description}</p>
            <button
